namespace UtilityPack.Actions
{
  ...
  [ContextAction(Name = "Use cast operator [Utility Pack]", Description = "Use cast operator.", Group = "C#")]
  public class UseCastOperator : ContextActionBase
  {
    ...
    public UseCastOperator(ICSharpContextActionDataProvider provider)
    {
    ...
    public override string Text
    {
      get
      {
        ...
        if (model == null)
        {
          return "Use cast operator [Utility Pack]";
        }
    ...
    {
      var expression = this.provider.GetSelectedElement<IAsExpression>(true, true);
      if (expression == null)
      {
        return null;
      }
      ...
      var code = string.Format("({0})({1})", typeOperand.GetText(), operand.GetText());
    ...
    private class Model
    {
      ...
      [NotNull]
      public IAsExpression Expression { get; set; }
